$PBExportHeader$w_std_pallet_label_armada.srw
forward
global type w_std_pallet_label_armada from Window
end type
end forward
...
global w_std_pallet_label_armada w_std_pallet_label_armada
...
lSerial = Long(Stparm.Value1)
   ...
   SELECT sum(object_b.quantity)
	INTO	:dQuantity  
    FROM  object object_b
   WHERE (:lSerial = object_b.parent_serial )  ; 
...
SELECT parameters.company_name, address_1, address_2, address_3
	...
	INTO		:szCustpart,
				:szdestination
	FROM		object pallet
				join object boxes on boxes.parent_serial = pallet.serial
				join shipper_detail on pallet.shipper = shipper_detail.shipper and
					boxes.part = shipper_detail.part_original
				join order_header on order_header.order_no = shipper_detail.order_no
	WHERE		pallet.serial = :lSerial  ; 
...
PrintSend ( ll_Label, cEsc + "V668" + cEsc + "H300" + cEsc + "B103095" + "*" + "S" + String(lSerial) + "*")
...
on w_std_pallet_label_armada.create
end on

on w_std_pallet_label_armada.destroy
end on
