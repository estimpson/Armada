...
--- <Body>
	...
	,	Part = xr.ChildPart
	,	BackflushingPrinciple = coalesce
			(	(	select
						msbpX.BackflushingPrinciple
					from
						dbo.MES_SetupBackflushingPrinciples msbpX
					where
						msbpX.Type = 3
						and msbpX.ID = xr.ChildPart
				)
			,	-1
			) --coalesce(msbpX.BackflushingPrinciple, -1)
		...
		(	select
			...
			where
				...
				left join dbo.location lPlant
					left join Fx.Globals defaultPlant
						on defaultPlant.Name = 'MES.DefaultPlant'
					join dbo.location lPlantMachines -- All the machines within the inventory's plant.
						on coalesce(lPlantMachines.plant, convert(varchar(10), defaultPlant.Value), 'N/A') = coalesce(lPlant.plant, convert(varchar(10), defaultPlant.Value), 'N/A')
