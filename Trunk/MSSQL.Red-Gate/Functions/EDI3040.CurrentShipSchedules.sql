...
CREATE FUNCTION [EDI3040].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDI3040.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDI3040.ShipSchedules ss
					ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			WHERE
				ssh.Status IN
				(	0 --(select dbo.udf_StatusValue('EDI3040.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDI3040.ShipScheduleHeaders', 'Status', 'Active'))
				)
			GROUP BY
				ss.ShipToCode
		...
		JOIN EDI3040.ShipScheduleHeaders ssh
			JOIN EDI3040.ShipSchedules ss
			ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			...
			LEFT JOIN
				EDI3040.BlanketOrders bo ON bo.EDIShipToCode = ss.ShipToCode
