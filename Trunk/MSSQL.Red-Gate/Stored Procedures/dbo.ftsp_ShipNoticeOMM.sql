...
CREATE PROCEDURE [dbo].[ftsp_ShipNoticeOMM]  (@shipper INT)
AS
BEGIN
...
/*
    ...
    FlatFile Layout for Overlay: MBS_856_D_v2040__150401    
...
DECLARE
	...
	@GrossWeightQualifier CHAR(2) ='G',
	@GrossWeightLbs CHAR(10),
	@NetWeightQualifier CHAR(2) = 'N',
	...
	SELECT
		...
		@SupplierName = 'Armada Rubber Manufacturing' ,
...
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('//STX12//856'+  @TradingPartner + @ShipperID2 + @PartialComplete ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('01'+  @PurposeCode + @ShipperID + @ASNDate + @ASNTime ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('02' +  @ShippedDate + @ShippedTime + @TimeZone   ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('03' +  @GrossWeightQualifier + @GrossWeightlbs + @WeightUM   ),1	
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('03' +  @NetWeightQualifier + @NetWeightlbs + @WeightUM   ),1	
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('04' +  @PackagingCode + @PackCount   ),1	
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('05' +  @SCAC + @TransMode ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('06' +  @EquipDesc + @EquipInitial + @TrailerNumber),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('07' + @REFMBQual + @REFBMValue ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('07' + @REFPKQual + @REFPKValue ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('08' + @SupplierQualifier + @SupplierIDType +  @SupplierCode ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('08' + @ShipFromQualifier + @SupplierIDType +  @SupplierCode ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('08' + @ShipToQualifier + @ShipToIDType +  @ShipToID ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('08' + @BillToQualifier + @BillToIDType +  @BillToCode ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('09' + @DockCode  ),1
INSERT	#ASNFlatFileHeader (LineData, DetailLineID)
	SELECT	('10' + @AETCResponsibility + @AETC  ),1
...
DECLARE	
	...
	@PackageType CHAR(5),
	@PackQty CHAR(12),
	@SerialPackQtyInt INT ,
	@PackQtyLoop INT,
	@SerialQualifier CHAR(2) = 'LS',
	@SerialNumber CHAR(16),
	@RCCount CHAR(12) = RTRIM(@PackCount)
...
WHILE
	...
	SELECT @PackQty = MAX(PackQty) FROM @ShipperSerials WHERE CustomerPart = @CustomerPartLoop
		...
		SELECT	('11'+ @CustomerpartBP + @CustomerPart + @CustomerPartEC + @CustomerECL  ),2

		INSERT	#FlatFileLines (LineData, DetailLineID)
		SELECT	('12'+ @ReturnableContainer + @QtyPacked + @UM + @AccumShipped),2

		INSERT	#FlatFileLines (LineData, DetailLineID)
		SELECT	('13'+  @CustomerPO  ),2
	

			DECLARE PackQty CURSOR LOCAL FOR
			SELECT	
				DISTINCT PackQty
									FROM
										@ShipperSerials
									...
									SELECT @PackQty = @SerialPackQtyInt
									
									INSERT	#FlatFileLines (LineData,  DetailLineID)
									SELECT	('14'+ @PackQty + @UM), 2
									...
									INSERT	#FlatFileLines (LineData,  DetailLineID)
									SELECT	('15'+ @SerialQualifier +  @SerialNumber   ), 2
...
INSERT	#FlatFileLines (LineData, DetailLineID)
		SELECT	('11'+ @CustomerRC + @ReturnableContainer  ),2

		INSERT	#FlatFileLines (LineData, DetailLineID)
		SELECT	('12'+ SPACE(30) + @RCCount + @UM ),2
