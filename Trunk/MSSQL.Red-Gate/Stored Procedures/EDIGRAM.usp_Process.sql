...
CREATE PROCEDURE [EDIGRAM].[usp_Process]
...
SET	@ProcName = USER_NAME(OBJECTPROPERTY(@@procid, 'OwnerId')) + '.' + OBJECT_NAME(@@procid)  -- e.g. EDIGRAM.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDIGRAM.CurrentPlanningReleases ()
...
END
--- <Update rows="*">
set	@TableName = 'EDIGRAM.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIGRAM.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIGRAM.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIGRAM.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIGRAM.ShipScheduleHeaders'
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIGRAM.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIGRAM.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIGRAM.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDIGRAM.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDIGRAM.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIGRAM.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIGRAM.ShipScheduleHeaders fh
...
from
	EDIGRAM.ShipScheduleHeaders fh
	join EDIGRAM.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIGRAM.ShipScheduleAccums fa
	...
	left join EDIGRAM.ShipScheduleAuthAccums faa
...
and	fh.Status = 1 --(select dbo.udf_StatusValue('EDIGRAM.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIGRAM.PlanningHeaders fh
	join EDIGRAM.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIGRAM.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIGRAM.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
				c.RawDocumentGUID = fr.RawDocumentGUID
	and	fh.Status = 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIGRAM.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIGRAM.PlanningHeaders fh
					join EDIGRAM.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIGRAM.ShipScheduleHeaders fh
					join EDIGRAM.ShipScheduleSupplemental sss
	...
	--- <Update rows="*">
		...
		join
		  ...
		  join
				EDIGRAM.PlanningSupplemental prs
		on
...
update
		...
		on
				bo.BlanketOrderNo = oh.order_no
		join
				(Select * From EDIGRAM.CurrentPlanningReleases()) cpr 
			on
				cpr.CustomerPart = bo.customerpart and
				cpr.ShipToCode = bo.EDIShipToCode and
				(	bo.CheckCustomerPOPlanning = 0
		...
		JOIN
			EDIGRAM.PlanningHeaders ph ON ph.RawDocumentGUID = cpr.RawDocumentGUID
		WHERE
		...
		on
				bo.BlanketOrderNo = oh.order_no
		join
				(Select * From EDIGRAM.CurrentShipSchedules()) css 
			on
		  ...
		  join
				EDIGRAM.ShipScheduleSupplemental sss
		on
	...
	/*update
		...
		from
			...
			join 
				EDIGRAM.ShipScheduleHeaders fh
			join 
				EDIGRAM.ShipSchedules ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIGRAM.BlanketOrders bo	on bo.EDIShipToCode =  ps.ShipToCode
					and bo.CustomerPart = ps.CustomerPart
...
and not exists
( Select 1 from 
		EDIGRAM.ShipSchedules b
 Join 
	EDIGRAM.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIGRAM.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIGRAM.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIGRAM.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDIGRAM.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for VISTEON-GRAMMER' 
...
execute
	@ProcReturn = EDIGRAM.usp_Process
	@TranDT = @TranDT out
