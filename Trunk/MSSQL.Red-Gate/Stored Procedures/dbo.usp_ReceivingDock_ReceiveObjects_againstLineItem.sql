...
update
	pd
set
	received = coalesce (received, 0) + dbo.udf_GetQtyFromStdQty(pd.part_number,  @PerBoxQty * @NewObjects, pd.unit_of_measure)
,	balance = coalesce (balance, quantity) - dbo.udf_GetQtyFromStdQty(pd.part_number,  @PerBoxQty * @NewObjects, pd.unit_of_measure)
,	standard_qty = standard_qty - @PerBoxQty * @NewObjects
,	last_recvd_date = @TranDT
,	last_recvd_amount = dbo.udf_GetQtyFromStdQty(pd.part_number,  @PerBoxQty * @NewObjects, pd.unit_of_measure)
from
	dbo.po_detail pd
where
	pd.po_number = @PONumber
...
execute
	@ProcReturn = dbo.usp_ReceivingDock_ReceiveObjects_againstLineItem
	@Param1 = @Param1
,	@TranDT = @TranDT out
,	@Result = @ProcResult out
