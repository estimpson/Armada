...
/*
Generate insert statements for all tables that have matching definitions and do not contain identity columns.
*/
...
from	(	select	TS.TableName,
			TableChecksum = CHECKSUM_AGG(BINARY_CHECKSUM(TS.TableName, TS.colorder, TS.name, TS.usertype, TS.length, TS.prec, TS.scale, TS.isnullable, TS.IsIdentity)),
			TableChecksumAlt = CHECKSUM_AGG(BINARY_CHECKSUM(TS.TableName, TS.colorder, TS.name, TS.usertype, TS.length, TS.prec, TS.scale, TS.isnullable))
			,	ColumnChecksum = checksum_agg(binary_checksum(RS.TableName, RS.name))
			,	HasIdentity = sum(TS.IsIdentity)
		from
			FxArmada.dbo.TableStructure TS
			left join rawArmada.dbo.RawStructure RS
				on RS.TableName = TS.TableName
				and RS.Name = TS.name
		group by
			TS.TableName) TableStructure
	join
	(	select	TableName,
			...
			,	ColumnChecksum = checksum_agg(binary_checksum(TableName, name))
...
where	TableStructure.TableChecksum != RawStructure.TableChecksum and
	...
	and TableStructure.ColumnChecksum = RawStructure.ColumnChecksum
	and TableStructure.HasIdentity = 0
order by
	1
