...
CREATE FUNCTION [EDIGRAM].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDIGRAM.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDIGRAM.ShipSchedules ss
					ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			WHERE
				ssh.Status IN
				(	0 --(select dbo.udf_StatusValue('EDIGRAM.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDIGRAM.ShipScheduleHeaders', 'Status', 'Active'))
				)
			GROUP BY
				ss.ShipToCode
		...
		JOIN EDIGRAM.ShipScheduleHeaders ssh
			JOIN EDIGRAM.ShipSchedules ss
			ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			...
			LEFT JOIN
				EDIGRAM.BlanketOrders bo ON bo.EDIShipToCode = ss.ShipToCode
