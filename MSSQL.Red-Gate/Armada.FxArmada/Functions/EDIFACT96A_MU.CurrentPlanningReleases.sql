...
CREATE FUNCTION [EDIFACT96A_MU].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIFACT96A_MU.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIFACT96A_MU.PlanningHeaders ph
				JOIN EDIFACT96A_MU.PlanningReleases pr
		...
		JOIN EDIFACT96A_MU.PlanningHeaders ph
			JOIN EDIFACT96A_MU.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIFACT96A_MU.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
