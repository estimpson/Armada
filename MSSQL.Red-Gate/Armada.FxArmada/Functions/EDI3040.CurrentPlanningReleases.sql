...
CREATE FUNCTION [EDI3040].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3040.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3040.PlanningHeaders ph
				JOIN EDI3040.PlanningReleases pr
		...
		JOIN EDI3040.PlanningHeaders ph
			JOIN EDI3040.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3040.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
