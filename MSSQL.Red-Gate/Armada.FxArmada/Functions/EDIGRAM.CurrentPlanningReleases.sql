...
CREATE FUNCTION [EDIGRAM].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIGRAM.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIGRAM.PlanningHeaders ph
				JOIN EDIGRAM.PlanningReleases pr
		...
		JOIN EDIGRAM.PlanningHeaders ph
			JOIN EDIGRAM.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIGRAM.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
