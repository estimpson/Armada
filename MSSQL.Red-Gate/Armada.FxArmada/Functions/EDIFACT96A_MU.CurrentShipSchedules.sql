...
CREATE FUNCTION [EDIFACT96A_MU].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDIFACT96A_MU.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDIFACT96A_MU.ShipSchedules ss
					ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			WHERE
				ssh.Status IN
				(	0 --(select dbo.udf_StatusValue('EDIFACT96A_MU.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDIFACT96A_MU.ShipScheduleHeaders', 'Status', 'Active'))
				)
			GROUP BY
				ss.ShipToCode
		...
		JOIN EDIFACT96A_MU.ShipScheduleHeaders ssh
			JOIN EDIFACT96A_MU.ShipSchedules ss
			ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			...
			LEFT JOIN
				EDIFACT96A_MU.BlanketOrders bo ON bo.EDIShipToCode = ss.ShipToCode
