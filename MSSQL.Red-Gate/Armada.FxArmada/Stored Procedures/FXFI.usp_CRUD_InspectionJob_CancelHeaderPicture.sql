...
create procedure [FXFI].[usp_CRUD_InspectionJob_CancelHeaderPicture]
	@User varchar(5)
...
begin
	...
	begin try
		...
		/*	Cancel header picture. */
		set @TocMsg = 'Cancel header picture'
		begin
			--- <Update rows="1">
			...
			set
				ijhp.Status = -1
			,	ijhp.RemovedByOperator = @User
			from
				FXFI.InspectionJobHeaderPictures ijhp
			where
				ijhp.InspectionJobNumber = @InspectionJobNumber
				and ijhp.RowID = @PictureID
...
execute
	@ProcReturn = FXFI.usp_CRUD_InspectionJob_CancelHeaderPicture
	@User = @User
,	@InspectionJobNumber = @InspectionJobNumber
,	@PictureID = @PictureID
,	@TranDT = @TranDT out
