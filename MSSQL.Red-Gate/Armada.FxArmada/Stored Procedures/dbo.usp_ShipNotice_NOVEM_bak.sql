...
CREATE PROCEDURE [dbo].[usp_ShipNotice_NOVEM_bak]  (@shipper INT)
AS
BEGIN

--dbo.usp_ShipNotice_NOVEM 361653
SET ANSI_PADDING ON
--ASN Header
...
DECLARE
	...
	@PackagingCodePAC CHAR(17) = 'CTN50'
	...
	SELECT
		...
		@ArrivalDate = CONVERT(CHAR, DATEADD(dd,2,GETDATE()), 112)  ,
		...
		@SoldToName =  d.name,
		@NADShipToID = COALESCE(da.shiptocode,es.EDIShipToID) ,
		@NADShipToName =  COALESCE(NULLIF(da.ShipToname,''), d.name),
		@NADShipToAdd1 =  COALESCE(NULLIF(da.ShipToAdd1,''), d.address_1),
		@NADShipToCityName =  COALESCE(da.ShipToCity,' '),
		@NADShipToState =  COALESCE(da.ShipToCountrySUB,' '),
		@NADShipToPostal =  COALESCE(da.ShipToPostalCode,' '),
		@NADShipToCountry =  COALESCE(da.ShipToCountry,' '),
		@NADSupplierCode = COALESCE(da.suppliercode, es.supplier_code),
		@NADSupplierName =  COALESCE(da.suppliername,' '),
		@NADSupplierAdd1 =  COALESCE(da.supplierAdd1,' '),
		@NADSupplierCityName =  COALESCE(da.supplierCity,' '),
		@NADSupplierState =  COALESCE(da.SupplierCountrySUB,' '),
		@NADSupplierPostal =  COALESCE(da.SupplierPostalCode,' '),
		@NADSupplierCountry =  COALESCE(da.SupplierCountry,' '),
		...
		@REFMAValue =COALESCE( ( Select top 1 release_no from shipper_detail where shipper = @shipper and nullif(release_no,'') is Not NULL ) , '')
	from
		Shipper s
	join
	...
	outer apply  (Select top 1 * from dbo.AddressFromDELFOR da where da.ShipToCode = es.EDIShipToID ) as da
					...
					SELECT  '12'
							...
							WHILE
								...
								END
									...
									INSERT	#ASNFlatFile (LineData)
									SELECT  '13' +  @PartPackCount + Space(3) + @PackagingCodePAC 
