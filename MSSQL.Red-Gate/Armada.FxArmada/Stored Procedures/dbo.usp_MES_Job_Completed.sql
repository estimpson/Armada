...
create procedure [dbo].[usp_MES_Job_Completed]
	@WODID int
,	@TranDT datetime out
,	@Result integer out
as
...
update
	wod
set
	Status = 1 --(select dbo.udf_StatusValue('dbo.WorkOrderDetails', 'Completed'))
from
	dbo.WorkOrderDetails wod
where
...
update
	woh
set
	Status = 1 -- (select dbo.udf_StatusValue('dbo.WorkOrderHeaders', 'Completed'))
from
...
execute
	@ProcReturn = dbo.usp_MES_Job_Done
	@Param1 = @Param1
,	@TranDT = @TranDT out
