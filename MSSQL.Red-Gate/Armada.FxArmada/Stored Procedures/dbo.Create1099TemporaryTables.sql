...
CREATE PROCEDURE [dbo].[Create1099TemporaryTables]
    ...
    @ai_year INT,
    @as_pay_vendor VARCHAR(25),
    @as_include_nec CHAR(1),
    @as_include_non_nec CHAR(1)

AS

-- 07-Dec-2018	1099s with an amount in Box 7 (NEC - non-employee compensation) are due
--		to the IRS by January 31.  All other 1099s are due by March 31.  Beginning
--		with the 2018 filing year, payers are penalized if they include forms
--		with NEC in a submission after January 31.  Therefore, we will let the
--		payer specify if he wants to include forms with NEC payments and/or
--		forms with no NEC payments.
--
--		Decided to include the Box 4s with the NECs if Box 7 has an amount;
--		otherwise, with the non-NEC. We can temporarily change this stored
--		procedure to get different results as needed.

--(Armada
-- 26-Jan-2015	Modified for Armada to produce a 1099 for code 7 income >= 500.
--Armada)
...
DECLARE sumcursor CURSOR FOR
    SELECT pay_vendor, code_1099,
     ...
     WHERE company = @as_company AND
           year_1099 = @ai_year AND
		   (pay_vendor = @as_pay_vendor OR @as_pay_vendor = '')
     GROUP BY pay_vendor, code_1099
    HAVING ((code_1099 = '1' AND sum(paid_amount) >= 600 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '2' AND sum(paid_amount) >= 10 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '3' AND sum(paid_amount) >= 600 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '4' AND sum(paid_amount) > 0 AND 
			   (((SELECT IsNull(sum(paid_amount),0)
                 ...
                 WHERE t_temp.company = @as_company AND
                       ...
                       code_1099 = '7') > 0 AND @as_include_nec= 'Y') OR
			   ((SELECT IsNull(sum(paid_amount),0)
                  FROM t1099_detail_temporary t_temp
                 WHERE t_temp.company = @as_company AND
                       t_temp.year_1099 = @ai_year AND
                       t_temp.pay_vendor = t1099_detail_temporary.pay_vendor AND
                       code_1099 = '7') = 0 AND @as_include_non_nec= 'Y'))) OR
           (code_1099 = '5' AND sum(paid_amount) > 0 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '6' AND sum(paid_amount) >= 600 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '7' AND (sum(paid_amount) >= 500 OR
               ...
               (SELECT IsNull(sum(paid_amount),0)
                 ...
                 WHERE t_temp.company = @as_company AND
                       t_temp.year_1099 = @ai_year AND
                       t_temp.pay_vendor = t1099_detail_temporary.pay_vendor AND
                       code_1099 = '4') > 0 )) AND @as_include_nec= 'Y') OR
           (code_1099 = '8' AND sum(paid_amount) >= 10 AND @as_include_non_nec= 'Y') OR
           (code_1099 = '9' AND sum(paid_amount) >= 5000 AND @as_include_non_nec= 'Y') OR
           (code_1099 = 'A' AND sum(paid_amount) >= 600 AND @as_include_non_nec= 'Y') OR
           (code_1099 = 'B' AND sum(paid_amount) > 0 AND @as_include_non_nec= 'Y') OR
           (code_1099 = 'C' AND sum(paid_amount) >= 600 AND @as_include_non_nec= 'Y'))
