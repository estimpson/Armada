...
create procedure [FXFI].[usp_CRUD_InspectionJob_Finalize]
	@User varchar(5)
...
begin
	...
	begin try
		...
		--- <Body>
		/*	If job requires a sort, create one. */
		set @TocMsg = 'If job requires a sort, create one'
		if	exists
			(	select
		  			*
		  		from
		...
		begin
			...
			,	@Description varchar(255) = 'Inspection of lot ' +
					(	select top(1)
							convert(varchar(13), ijo.Serial)
						from
							FXFI.InspectionJobObjects ijo
						where
							ijo.InspectionJobNumber = @InspectionJobNumber
			...
			execute @ProcReturn = dbo.usp_InventoryControl_QualityBatch_NewHeader
				@User = @User
			,	@Description = @Description
			,	@QualityBatchNumber = @QualityBatchNumber out
			,	@TranDT = @TranDT out
			,	@Result = @Result out
			...
			while
				...
				execute @ProcReturn = dbo.usp_InventoryControl_QualityBatch_AddObject
						@User = @User
		...
		begin
			...
			update
				ijh
			set
				ijh.InspectionStatus = 'FINALIZED'
			,	ijh.Status = 1
			from
...
execute
	@ProcReturn = FXFI.usp_CRUD_InspectionJob_Finalize
	@User = @User
,	@PartCode = @PartCode
