...
declare
	...
	select
		RawDocumentGUID = ed.GUID
	,	ReleaseNo =		coalesce(	ed.fulldata.value('(/TRN-DELJIT/SEG-BGM/CE/DE[@code="1004"])[1]', 'varchar(50)'),'')
...
declare
	...
	select
		RawDocumentGUID = ed.GUID
	,	ReleaseNo =		coalesce(	ed.fulldata.value('(/TRN-DELJIT/SEG-BGM[1]/CE/DE[@code="1004"])[1]', 'varchar(50)'),'')
...
declare
	...
	declare
		@PlanningReleaseSupplementalTemp2 table
	(	RawDocumentGUID uniqueidentifier
	,	ReleaseNo varchar(50)
	,	ShipToCode varchar(15)
	,	ConsigneeCode varchar(15)
	...
	declare
		@tempPlanningReleaseSupplemental11 table
	(	RawDocumentGUID uniqueidentifier
	,	ReleaseNo varchar(50)
	,	ShipToCode varchar(15)
	,	ConsigneeCode varchar(15)
	...
	declare
		@tempPlanningReleaseSupplemental12 table
	(	RawDocumentGUID uniqueidentifier
	,	ReleaseNo varchar(50)
	,	ShipToCode varchar(15)
	,	ConsigneeCode varchar(15)
	...
	,	ShipToCode =	coalesce(	EDIData.Releases.value('(../LOOP-NAD/SEG-NAD [DE[.="ST"][@code="3035"]]/CE/DE[@code="3039"])[1]', 'varchar(15)')	,ed.fulldata.value('(/TRN-DELFOR/LOOP-NAD/SEG-NAD [DE[.="ST"][@code="3035"]]/CE/DE[@code="3039"])[1]', 'varchar(15)'))
	...
	from
		EDI.EDIDocuments ed
		cross apply ed.fulldata.nodes('/TRN-DELFOR/LOOP-GIS/LOOP-LIN') as EDIData(Releases)
	where
		ed.HeaderData.value('/TRN-INFO[1]/@type', 'varchar(50)') = 'DELFOR'
		and ed.EDIStandard = '97A'
		and ed.Status = 100 -- (select dbo.udf_StatusValue('EDI.EDIDocuments', 'InProcess'))
		and ed.TradingPartner not like 'FAU%'

	insert
		@tempPlanningReleaseSupplemental11
	(	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
	...
	from
		EDI.EDIDocuments ed
		cross apply ed.fulldata.nodes('/TRN-DELFOR/LOOP-GIS') as EDIData(Releases)
	where
		ed.HeaderData.value('/TRN-INFO[1]/@type', 'varchar(50)') = 'DELFOR'
		and ed.EDIStandard = '97A'
		and ed.Status = 100 -- (select dbo.udf_StatusValue('EDI.EDIDocuments', 'InProcess'))
		and ed.TradingPartner like 'FAU%'
	...
	,	8
		...
		insert
		@tempPlanningReleaseSupplemental12
	(	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
	...
	,	ShipFromCode
	,	SupplierCode
	,	CustomerPart	=	coalesce(nullif((Data.value('(for $a in LOOP-GIS/LOOP-LIN/SEG-LIN/CE/DE[@code="7143"] where $a="BP" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')),''), (Data.value('(for $a in LOOP-GIS/LOOP-LIN/SEG-LIN/CE/DE[@code="7143"] where $a="IN" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')) )
	,	CustomerPO		=	coalesce(nullif((Data.value('(for $a in LOOP-GIS/LOOP-LIN/SEG-PIA/CE/DE[@code="7143"] where $a="PO" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')),''),(Data.value('(for $a in LOOP-GIS/LOOP-LIN/LOOP-RFF/SEG-RFF/CE/DE[@code="1153"] where $a="ON" return $a/../DE[. >> $a][@code="1154"][1])[1]', 'varchar(50)')) )
	,	CustomerPOLine	=	Data.value('(for $a in LOOP-GIS/LOOP-LIN/SEG-PIA/CE/DE[@code="7143"] where $a="PL" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')
	,	CustomerModelYear = ''
	,	CustomerECL		=	''
	,	Data = EDIData.ps.query('.')
	
	from
		@tempPlanningReleaseSupplemental11 as ps
		cross apply ps.data.nodes('/LOOP-GIS') as EDIData(ps)
	order by
		2
	,	3
	,	8
	...
	Insert
		@PlanningReleaseSupplementalTemp2
	(	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
	...
	,	CustomerModelYear
	,	CustomerECL
	,	ReferenceType = data.value('(/LOOP-GIS/LOOP-NAD/SEG-LOC/DE[@code="3227"])[1]', 'varchar(15)')
	,	Reference = data.value('(/LOOP-GIS/LOOP-NAD/SEG-LOC/CE/DE[@code="3225"])[1]', 'varchar(15)')
	From
		@tempPlanningReleaseSupplemental12 
...
UNION
	...
	,	CustomerModelYear
	,	CustomerECL 
	,	UserDefined1 = max(case when ssst.ReferenceType = '11' then ssst.Reference end)
	,	UserDefined2 = max(case when ssst.ReferenceType = 'LF' then ssst.Reference end)
	,	UserDefined3 = max(case when ssst.ReferenceType = 'RL' then ssst.Reference end)
	,	UserDefined4 = max(case when ssst.ReferenceType = '11' then ssst.Reference end)--Qualifier Type of 11
	,	UserDefined5 = max(case when ssst.ReferenceType = '159' then ssst.Reference end)--Qualifier Type of 159
	...
	,	UserDefined20 = max(case when ssst.ReferenceType = '??' then ssst.Reference end)
	from
		@PlanningReleaseSupplementalTemp2 ssst
	group by
		RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
...
--Select * From @tempPlanningReleaseSupplemental11
--Select * From @TempPlanningReleaseSupplemental12
--Select * From @PlanningReleaseSupplementalTemp2
...
declare
	...
	,	CustomerPart	=	coalesce(nullif((Data.value('(for $a in LOOP-LIN/SEG-LIN/CE/DE[@code="7143"] where $a="BP" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')),''), (Data.value('(for $a in LOOP-LIN/SEG-LIN/CE/DE[@code="7143"] where $a="IN" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')) )
	,	CustomerPO		=	coalesce(nullif((Data.value('(for $a in LOOP-LIN/SEG-PIA/CE/DE[@code="7143"] where $a="PO" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)')),''),(Data.value('(for $a in LOOP-LIN/LOOP-RFF/SEG-RFF/CE/DE[@code="1153"] where $a="ON" return $a/../DE[. >> $a][@code="1154"][1])[1]', 'varchar(50)')) )
	,	CustomerPOLine	=	coalesce(Data.value('(for $a in LOOP-LIN/SEG-PIA/CE/DE[@code="7143"] where $a="PL" return $a/../DE[. << $a][@code="7140"][1])[1]', 'varchar(50)') , (Data.value('(for $a in LOOP-LIN/LOOP-RFF/SEG-RFF/CE/DE[@code="1153"] where $a="ON" return $a/../DE[. >> $a][@code="1156"][1])[1]', 'varchar(50)')))
	,	CustomerECL =''
	,	CustomerModelYear = ''
	,	Data = EDIData.Releases.query('.')
	from
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingShipScheduleHeaders'

	insert
		FxArmada.EDIEDIFACT97A.StagingShipScheduleHeaders
	(	RawDocumentGUID
	,	DocumentImportDT
	,	TradingPartner
	,	DocType
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingShipScheduleSupplemental'
	
	insert 
		FxArmada.EDIEDIFACT97A.StagingShipScheduleSupplemental
	(	RawDocumentGUID
    ,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingShipScheduleAccums'

	insert  FxArmada.EDIEDIFACT97A.StagingShipScheduleAccums
    (	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingShipScheduleAuthAccums'

	insert
		FxArmada.EDIEDIFACT97A.StagingShipScheduleAuthAccums
	(	
		RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingShipSchedules'

	insert
		FxArmada.EDIEDIFACT97A.StagingShipSchedules
	(	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingPlanningHeaders'

	insert
		FxArmada.EDIEDIFACT97A.StagingPlanningHeaders
	(	RawDocumentGUID
	,	DocumentImportDT
	,	TradingPartner
	,	DocType
...
if	exists
	...
	insert 
		FxArmada.EDIEDIFACT97A.StagingPlanningSupplemental
	(	RawDocumentGUID
    ,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingPlanningAccums'

	insert
		FxArmada.EDIEDIFACT97A.StagingPlanningAccums
	 (	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
    ...
    ,	ReferenceNo = max(case when QuantityQualifier = '48' then coalesce(UserDefined10,'') end)
    ,   LastQtyReceived = max(case when QuantityQualifier = '48' then coalesce(nullif(Quantity,''),0) end)
    ,   LastQtyDT = max(case when QuantityQualifier = '48' and DateDT is NOT NULL then case DateDTFormat  --added and DateDT is NOT NULL 2/9/2018 asb FT, LLC
					when '102' Then dbo.udf_GetDT('CCYYMMDD', coalesce(nullif(DateDT, '00000000'), '20990101'))
					when '103' Then dbo.udf_GetDT('CCYYWW', coalesce(nullif(DateDT, '00000000'), '209901'))
					when '203' Then dbo.udf_GetDT('CCYYMMDDHHMM', coalesce(nullif(DateDT, '00000000'), '209901010001'))
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingPlanningAuthAccums'

	insert
		FxArmada.EDIEDIFACT97A.StagingPlanningAuthAccums
	(	
		RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxArmada.EDIEDIFACT97A.StagingPlanningReleases'

	insert
		FxArmada.EDIEDIFACT97A.StagingPlanningReleases
	(	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
select
	*
from
	FxArmada.EDIEDIFACT97A.StagingShipScheduleHeaders sfh

Select 'StagingSSchedules'
select
	*
from
	FxArmada.EDIEDIFACT97A.StagingShipSchedules sfr

Select 'StagingSSAccums'
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingShipScheduleAccums sfs

Select 'StagingSSAuthAccums'
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingShipScheduleAuthAccums sfs

Select 'StagingSSSupplemental'
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingShipScheduleSupplemental sfs
go
...
select
	*
from
	FxArmada.EDIEDIFACT97A.StagingPlanningHeaders sfh

Select 'PlanningReleases'
select
	*
from
	FxArmada.EDIEDIFACT97A.StagingPlanningReleases sfr

Select 'PlanningAccums'	
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingPlanningAccums sfa

Select 'PlanningAuthAccums'	
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingPlanningAuthAccums sfa
Select 'PlanningSupplemental'	
select 
	*
from
	FxArmada.EDIEDIFACT97A.StagingPlanningSupplemental sfa
