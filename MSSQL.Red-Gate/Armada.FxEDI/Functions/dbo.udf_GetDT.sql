...
--- <Body>
	declare
		@Result datetime,
		@ConvertedRawValue varchar(12)

	Set @ConvertedRawValue =
		case @Format
			when 'YYDDMM' then
				substring(@RawValue,1,2) + substring(@RawValue,5,2) + substring(@RawValue,3,2)
		end
	...
	set	@Result =
		case @Format
			...
			when 'YYDDMM' then
				convert(datetime, @ConvertedRawValue)
			when 'YYMMDD' then
				convert(datetime, @RawValue)
