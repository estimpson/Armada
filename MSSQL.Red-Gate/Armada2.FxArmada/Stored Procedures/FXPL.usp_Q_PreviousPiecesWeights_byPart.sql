...
create procedure [FXPL].[usp_Q_PreviousPiecesWeights_byPart]
	@PartCode varchar(50)
			...
			select
				result =
					...
					(	(	select top(10)
							...
							where
								pjh.Status > 0
								and pjh.PartCode = @PartCode
							order by
								pjh.RowCreateDT desc
							for xml raw('RecentPieceWeight'), type
						),
						convert(xml, '')
					)
...
execute
	@ProcReturn = FXPL.usp_Q_PreviousPiecesWeights_byPart
	@PartCode = @PartCode
,	@TranDT = @TranDT out
