...
create procedure [FXFI].[usp_CRUD_InspectionJob_SerialScanned]
	@User varchar(5)
...
begin
	...
	begin try
		...
		/*	Update status of scanned object. */
		set @TocMsg = 'Update status of scanned object'
		begin
			--- <Update rows="1">
						...
						when not exists
							(	select
								...
								where
									ijo.InspectionJobNumber = @InspectionJobNumber
									and ijo.Status >= 0
									and ijo.InspectionStatus = 'scanned'
							) then 'scan complete'
						else 'scan begun'
					end
			,	Status =
		...
		begin
			--- <Call>	
			set	@CallProcName = 'FXFI.usp_Q_InspectionJob_byJobNumber'
	
			execute @ProcReturn = FXFI.usp_Q_InspectionJob_byJobNumber
					@InspectionJobNumber = @InspectionJobNumber
				,	@TranDT = @TranDT out
...
declare
	@User varchar(5) = 'EES'
,	@InspectionJobNumber varchar(50) = 'PJ_000000035'
...
execute
	@ProcReturn = FXFI.usp_CRUD_InspectionJob_SerialScanned
	@User = @User
,	@InspectionJobNumber = @InspectionJobNumber
,	@Serial = @Serial
,	@TranDT = @TranDT out
