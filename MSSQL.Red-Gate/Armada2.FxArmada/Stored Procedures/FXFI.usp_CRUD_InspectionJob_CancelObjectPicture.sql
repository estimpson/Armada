...
create procedure [FXFI].[usp_CRUD_InspectionJob_CancelObjectPicture]
	@User varchar(5)
...
begin
	...
	begin try
		...
		/*	Cancel object picture. */
		set @TocMsg = 'Cancel object picture'
		begin
			--- <Update rows="1">
			...
			set
				ijop.Status = -1
			,	ijop.RemovedByOperator = @User
			from
				FXFI.InspectionJobObjectPictures ijop
			where
				ijop.InspectionJobNumber = @InspectionJobNumber
				and ijop.RowID = @PictureID
...
execute
	@ProcReturn = FXFI.usp_CRUD_InspectionJob_CancelObjectPicture
	@User = @User
,	@InspectionJobNumber = @InspectionJobNumber
,	@Serial = @Serial
,	@PictureID = @PictureID
